...
class Loss():
    ...
    def loss_function(self, recon_x, x, encoded, centroids, assignments, mu=None, logvar=None):
        ...
        assert encoded.shape == centroids.shape, "Dimension does not match centroid and encoded"
        diff = torch.norm(encoded - centroids, dim=1)
        regularization_loss = torch.mean(diff)
